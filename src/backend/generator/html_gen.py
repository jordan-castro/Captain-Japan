from src.backend.utils.dir_helper import hide_dir
...
def generate_html_file(data, title, directory):
    ...
    with open(file_path, "w", encoding="utf-8") as html_file:
        ...
        # Write the title with a nice looking blue-grey divider
        html_file.write(f'<center><h1>{title.replace("_", " ")}</h1></center>')
        html_file.write('<br><hr style="border-top: 6px solid #7393B3; border-radius: 5px; width: 20%">')
        # Write the html
